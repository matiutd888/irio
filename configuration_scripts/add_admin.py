...
def add_admin_data(cursor, admin):
    try:
        if all(key in admin.keys() for key in ['admin_id', 'telegram_contact_id', 'phone_number', 'email_address']):
            cursor.execute(
            ...
            print("Error: Incomplete admin information. Provide admin_id, telegram_contact_id, phone_number, and email_address for each admin.")
...
def update_admin_data(cursor, admin):
    try:
        ...
        update_values = {key: admin[key] for key in admin.keys() if key != 'admin_id' and admin[key] is not None}
        ...
        if admin_id and update_values:
            ...
            if existing_admin:
                ...
                cursor.execute(update_query, tuple(update_values.values()) + (admin_id,))
...
def main():
    ...
    parser.add_argument('--telegram-contact-id', required=False, help='telegram contact ID of the admin')
